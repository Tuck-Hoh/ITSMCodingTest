...
function displayRecords(preselectId) {
    ...
    if (addressBookRecords.length > 0) {
        for (i = 0; i < addressBookRecords.length; i++) {
            displayData = displayData + addressBookRecords[i].firstName + "," + addressBookRecords[i].lastName + "</br>";
        }
    }
    document.getElementById("showList").innerHTML = displayData;
}

// Adds a new entry with the First and Last name of "New"
function addNewEntry() {
    // << YOUR CODE HERE >>
    document.getElementById("editableDisplay").style.display = "block";

    document.getElementById("inputFirstName").value = "New";
    document.getElementById("inputLastName").value = "New";

    var p = { "firstName": document.getElementById("inputFirstName").value, "lastName": document.getElementById("inputLastName").value }; 
...
function saveEntry() {
    ...
    var p = { "fieldValue1": "frm_Template_ProductCategoryTier2" };
    alert(p);
    $.ajax({
        url: '/Home/SaveEntry',
        type: "POST",
